...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		server: {
			description:
				"Sandbox Google MCP for Cursor Pro masterclass. Experimental account: ckoolwal@gmail.com only. Workspace tools (Gmail, Drive, Calendar, Contacts, Tasks, Sheets, Docs).",
...
async function redirectToGoogle(
	...
	// Workspace-class scopes for the experimental sandbox Google account.
	// Enable these APIs in the Google Cloud project and add ckoolwal@gmail.com as a test user.
	const scope = [
		"openid",
		"email",
		"profile",
...
app.get("/callback", async (c) => {
	...
	// Sandbox firewall: only the experimental Google account.
	if (email?.toLowerCase() !== "ckoolwal@gmail.com") {
		return c.text(
			`Access denied. Sandbox Google MCP only allows ckoolwal@gmail.com (signed in as ${email ?? "unknown"}).`,
			403,
		);
	}
